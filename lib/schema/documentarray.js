...
function DocumentArray(key, options) {
  ...
  if (schema.name === 'Schema' || schema.constructor.name === 'Object') {
    ...
    this.DocArray.prototype._cast = function (value) {
      ...
      if (!val._parent) {
        val._parent = this._parent;
        val._parentPath = _this.path;
      }
  ...
  // else override modelArray_cast with type#cast
    ...
    Type = new DocumentArray.getSchemaType(schema);
    ctx = { options: options.type[0] };

    if (options.ref) ctx.options.ref = options.ref;

    this.DocArray.prototype._cast = function (value) {
      var val = Type.prototype.cast.call(ctx, value);

      if (ctx.options
       && ctx.options.ref
       && val instanceof DocumentArray.model(ctx.options.ref)) {
